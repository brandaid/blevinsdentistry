(function($) {
	...
	var lazy_to_img_url = function(subj_id, is_coll) {
		$subj = (typeof(is_coll) == 'undefined') ? jQuery('#'+subj_id+ ' .gg_main_thumb') : jQuery('#'+subj_id+ ' .gg_coll_outer_container .gg_main_thumb');
		
		$subj.each(function() {
			if(jQuery(this).attr('gg-lazy-src') != 'undefined') {
				jQuery(this).attr('src', jQuery(this).attr('gg-lazy-src'));
				jQuery(this).removeAttr('gg-lazy-src');
			}
		});
	};
	...
	gg_man_standard_gallery = function(gid) {
		if(!jQuery('#'+gid+' .gg_img').length) {return false;}
		lazy_to_img_url(gid);
	...
	var layout_standard_gall = function(gid, has_new_img) {
		...
		while(row_w <= gg_gallery_w[gid]) {
			...
			if(img_per_row == jQuery('#'+gid+' .gg_img').length) {break;} // safety block
		}
	...
	var layout_photostr_gall = function(gid) {
		...
		// use gg_lor on last image in last row
		if(gg_temp_w[gid]) {
			jQuery('#'+gid+' .gg_img').last().removeClass('gg_lor');	
		}
	}
	...
	var photostr_row_img_shrink = function(gid, to_shrink) {
		...
		if(gg_row_img[gid].length == 1) {
			jQuery(gg_row_img[gid][0]).css('width', '100%');
			return true;
		}
		...
		for(a=0; a < gg_row_img[gid].length; a++) {
			...
			if((gg_row_img_w[gid][a] - proport_shrink) >= gg_phosostr_min_w || a == (gg_row_img[gid].length - 1)) {
				var img_w = gg_row_img_w[gid][a] - proport_shrink;
				jQuery(gg_row_img[gid][a]).css('width', img_w );	
				
				new_row_w = new_row_w + (gg_row_img_w[gid][a] - proport_shrink);
				remaining_shrink = remaining_shrink - proport_shrink;
			}
			else {
				var new_w = gg_phosostr_min_w;
				
				// if min width is bigger than image's width
				if(gg_row_img_w[gid][a] < new_w) {
					new_w = gg_row_img_w[gid][a];
					jQuery(gg_row_img[gid][a]).css('width', new_w );
			...
			if(a == (gg_row_img[gid].length - 1)) {
				...
				if(diff > 0) {
					jQuery(gg_row_img[gid][a]).css('width', jQuery(gg_row_img[gid][a]) - diff);	
				} 
				else if(diff < 0) {
					jQuery(gg_row_img[gid][a]).css('width', jQuery(gg_row_img[gid][a]) + diff);
				}
	...
	gg_load_coll_gallery = function(cid, gdata) {
		...
		coll_scroll_helper[cid] = jQuery('#'+cid+' .gg_coll_img[gall-data="'+gdata+'"]');
		
		// prepare
		if( jQuery('#'+cid+' .gg_coll_gallery_container .gg_gallery_wrap').length) {
	...
	var fill_coll_gallery = function(cid, gall_data) {
		jQuery('#'+cid+' .gg_coll_gallery_container .gg_gallery_wrap').remove();
		jQuery('#'+cid+' .gg_coll_gallery_container').removeClass('gg_main_loader').append(gall_data);
		
		if(jQuery('#'+cid+' .gg_coll_gall_title').length > 1) {
			jQuery('#'+cid+' .gg_coll_gall_title').first().remove();
		}
	...
	jQuery(document).ready(function() {
		jQuery('body').delegate('.gg_coll_go_back', 'click', function() {
			...
			if((elemBottom > docViewBottom) || elemTop < docViewTop) {
				...
				jQuery('html, body').animate({'scrollTop': coll_top_pos}, 600);	
			}
			
			// go back
			jQuery('#'+cid+' .gg_coll_table_first_cell').css('opacity', 1);		
			jQuery('#'+cid+' > table').animate({'left' : 0}, 700);
		...
		if( jQuery('.gg_filter').length ) {
			jQuery('.gg_gallery_wrap').each(function() {
				...
				if(hash.indexOf('#gg_cf') !== -1) {
					if( jQuery('#'+cid+' .gg_filter a[rel=' + val + ']').length ) {
	...
	function gg_get_cg_deeplink(browser_history) { // coll selection
		...
		if(hash.indexOf('#gg_cg') !== -1) {
			...
			if( jQuery('.gg_coll_img[rel=' + gid + ']').length ) {
		...
		// trigger filter
			...
			setTimeout(function() {
				jQuery('.ggf_id_'+gg_coll_dl_filter).trigger('click');
	...
	function gg_set_deeplink(subj, val) {
		if( gg_use_deeplink ) {
			...
			// add text
			if(subj == 'cat') {
				var txt = jQuery('.ggf_id_'+val).text();
			} else {
				var txt = (jQuery('.gg_coll_img[rel='+val+']').parents('.gg_coll_img_wrap').find('.gg_img_title_under').length) ? jQuery('.gg_coll_img[rel='+val+']').parents('.gg_coll_img_wrap').find('.gg_img_title_under').text() : jQuery('.gg_coll_img[rel='+val+']').find('.gg_img_title').text();
			}
	...
	jQuery(document).ready(function() {
		...
		jQuery(document).ready(function() {
			jQuery('body').delegate('.gg_num_btns_wrap > div', 'click', function() {
				var gid = jQuery(this).parents('.gg_gallery_wrap').attr('id');
				
				if( !jQuery(this).hasClass('gg_pag_disabled') && gg_is_paginating[gid] == 0 ){
					gg_standard_pagination(gid, jQuery(this).attr('rel'));
				}
			});	
		});
		...
		// numbered buttons - handle click
		jQuery(document).ready(function() {
			jQuery('body').delegate('.gg_dots_pag_wrap > div', 'click', function() {
				var gid = jQuery(this).parents('.gg_gallery_wrap').attr('id');
	...
	gg_standard_pagination = function(gid, new_pag) {
		if(gg_is_paginating[gid] == 0) {
			...
			if(jQuery('#'+gid+' .gg_standard_pag').length) {
				if(new_pag >= parseInt(jQuery('#'+gid+' .gg_paginate').attr('gg-totpages'))) { jQuery('#'+gid+' .gg_paginate').find('.gg_next_page').addClass('gg_pag_disabled'); }
				else { jQuery('#'+gid+' .gg_paginate').find('.gg_next_page').removeClass('gg_pag_disabled'); }
				
				if(new_pag == 1) { jQuery('#'+gid+' .gg_paginate').find('.gg_prev_page').addClass('gg_pag_disabled'); }
				else { jQuery('#'+gid+' .gg_paginate').find('.gg_prev_page').removeClass('gg_pag_disabled'); }
			}
			// manage for numbered buttons and dots
			else {
				jQuery('#'+gid+' .gg_num_btns_wrap > div, #'+gid+' .gg_dots_pag_wrap > div').removeClass('gg_pag_disabled');
				jQuery('#'+gid+' .gg_num_btns_wrap > div[rel='+ new_pag +'], #'+gid+' .gg_dots_pag_wrap > div[rel='+ new_pag +']').addClass('gg_pag_disabled');	
			}
		
			// check in cache
			if(typeof(gg_gall_pag_cache[gid][new_pag]) != 'undefined') {
				fill_standard_pagination(gid, gg_gall_pag_cache[gid][new_pag], new_pag, smooth_timing); 
			}
			else {
				// get new datas
				var data = {
				...
				jQuery.post(window.location.href, data, function(response) {
					gg_gall_pag_cache[gid][new_pag] = response;
					fill_standard_pagination(gid, response, new_pag, smooth_timing); 
				});
			}
		}
	};
	...
	var fill_standard_pagination = function(gid, data, new_pag, delay) {
		setTimeout(function() {
	...
	jQuery(document).ready(function() {
		jQuery('body').delegate('.gg_infinite_scroll', 'click', function() {
			...
			if( jQuery('#'+gid+' .gg_paginate .gg_loader').length ) {jQuery('#'+gid+' .gg_paginate .gg_loader').remove();}
	...
	gg_check_primary_ol = function(gid, respect_delay) {		
		jQuery('#'+gid+' .gg_img').each(function(i, e) {
			...
			if(!jQuery.trim($ol_subj.find('.gg_img_title').html())) {
	...
	var gg_keep_back_to_gall_visible = function() {
		if( jQuery('.gg_coll_back_to_new_style').length && typeof(gg_back_to_gall_scroll) != 'undefined' && gg_back_to_gall_scroll) {
	...
	// via image click
	jQuery(document).ready(function() {
		jQuery(document).delegate('.gg_gallery_wrap div.gg_img:not(.gg_coll_img, .gg_linked_img)', 'click', function(e) {	
			e.preventDefault();	
			...
			var clicked_url = $clicked.attr('gg-url');
			var clicked_index = 0;
			
			jQuery('#'+gid+' .gg_img').each(function(i, v) {
				var img_url = jQuery(this).attr('gg-url');
				
				if(typeof( gall_obj[img_url] ) == 'undefined') {
					gall_obj[img_url] = {
						"img"		: img_url,
						"title"		: gg_lb_html_fix(jQuery(this).attr('gg-title')),
						"descr"		: gg_lb_html_fix(jQuery(this).attr('gg-descr')),
						'author'	: gg_lb_html_fix(jQuery(this).attr('gg-author'))
					};	
