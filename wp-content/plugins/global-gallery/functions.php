...
function gg_types($type = false) {
	...
	if($php_ver < 5.3 || PHP_INT_SIZE === 4) {unset($types['dropbox']);}
...
function gg_frontend_img_split($gid, $images, $type, $selection = 'all', $randomized_order = false, $watermark = false) {
	$to_return = array();
	...
	for($a=$start; $a<$end; $a++) {
		if(!isset($randomized_order[$a])) {break;}
		...
		// apply watermark	
		if($apply_watermark) {
			$orig_src = gg_img_src_on_type($img['img_src'], $type);
			
			if(trim($orig_src)) {
				$new_paths = gg_watermark($orig_src);
				
				$img['wm_url'] = $new_paths['url'];
				$img['wm_path'] = $new_paths['path'];
			}	
		}
			
		// prepare image's array
		if(get_post_meta($gid, 'gg_autopop', true)) {
			if(!get_post_meta($gid, 'gg_auto_title', true)) 	{$img['title'] = '';}
			if(!get_post_meta($gid, 'gg_auto_author', true)) 	{$img['author'] = '';}
			if(!get_post_meta($gid, 'gg_auto_descr', true)) 	{$img['descr'] = '';}  
