...
function gg_load_settings() {
	...
	// error cases
	if( (($type != 'wp' && $type != 'wp_cat' && $type != 'cpt_tax' && $type != 'gg_album' && $type != 'ngg') && trim($username) == '') || ($type == 'picasa' && trim($psw) == '')) {
	...
	// v4.2 - if google drive - forces autopopulation
	if($type == 'g_drive') {
		$autopop = 1;
		$disable_autopop_switch = 'disabled="disabled"';	
	}
	else {$disable_autopop_switch = '';}

	// switches
	$autopop_fields = ($autopop == 1) ?  '' : 'style="display: none;"';
	...
	if($type != 'wp') {
		...
		<h4>' . __('Auto Gallery Population', 'gg_ml') . '</h4>
		  ...
		  <tr>
			...
			<td class="lcwp_field_td" id="gg_autopop"><input type="checkbox" value="1" name="gg_autopop" class="ip-checkbox" '.$disable_autopop_switch.' '.gg_checkbox_check($autopop).' /></td>     
	...
	foreach($to_save as $img) {
		...
		echo '<li>
			<div class="gg_builder_img_wrap">
				<figure style="background-image: url('.$thumb_url.');" class="gg_builder_img" fullurl="'.$img['url'].'"></figure>
			</div>	
			<div>
				<table>
				  <tr>
...
function gg_cpt_tax_change() {
	if(!isset($_POST['cpt_tax'])) {die('data is missing');}
