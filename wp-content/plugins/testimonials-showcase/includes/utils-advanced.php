...
if (!function_exists('cmshowcase_get_featured_img')) {
	...
	function cmshowcase_get_featured_img( $id, $size = 'thumbnail', $email = '', $use_default = true, $default = '', $dsize = array(), $gravatar = true) {
...
if (!function_exists('cmshowcase_build_pager')) {
	...
	function cmshowcase_build_pager($cpt,$query,$labels = array(), $type = 'on') {
		...
		if($max_page>1) {
			...
			$previous_page = __("Previous Page",$cpt);
			$next_page = __("Next Page",$cpt);

			if(!empty($labels)) {
				$previous_page = isset($labels['previous']) ? $labels['previous'] : $previous_page;
				$next_page = isset($labels['next']) ? $labels['next'] : $previous_page;
			}
