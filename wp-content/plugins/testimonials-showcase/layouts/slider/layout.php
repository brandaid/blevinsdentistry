...
class cm_tt_slider {
		...
		$settings = array(
						...
						'fields' => array(
							...
							'next-arrow-code' => array(
								...
								'description' => __('Code to display for Next Arrow. By Default we use a <a href="http://fontawesome.io/icons/#directional" target="_blank">Font Awesome Icon</a>.','ttshowcase'),
								'type' => 'text',
							...
							'prev-arrow-code' => array(
								...
								'description' => __('Code to display for Previous Arrow. By Default we use a <a href="http://fontawesome.io/icons/#directional" target="_blank">Font Awesome Icon</a>.','ttshowcase'),
								'type' => 'text',
		...
		$options = array(
			...
			'controls' => array(
					...
					'options' => array(
						...
						'controls' => __('Next / Previous Arrows', 'ttshowcase'),
			...
			'review_title' => array(
				'label' => __('Testimonial Title','ttshowcase'),
				'description' => __('Display Testimonial/Review title','ttshowcase'),
				'type' => 'checkbox',
				'default' => 'off',
				'value' => 'on'
				),
	...
	public function build_layout( $query = array() , $options = array(), $preview = false ) {
		...
		//Variables from the ADVANCED SETTINGS PAGE
			...
			$info_elements = cmshowcase_get_option( 'info_elements', $advanced_section, '{author}{subtitle}{stars}{date}' ); 
		...
		//Variables from the GRID LAYOUT OPTIONS PAGE
			...
			//%%SLIDER SPECIFIC
			$next_arrow = cmshowcase_get_option( 'next-arrow-code', $section, '<i class="fa fa-caret-right fa-2x" ></i>' );
			$prev_arrow = cmshowcase_get_option( 'prev-arrow-code', $section, '<i class="fa fa-caret-left fa-2x" ></i>' );
		...
		$html = '<div class="ttshowcase_slider" style="display:none">';
        ...
        if ( $query->have_posts() ) {
			...
			while ( $query->have_posts() ) {
				...
				//if everything is ok, proceed
					...
					//Image Process
						...
						if(isset($options['image-size-override']) && $options['image-size-override'] != '' && is_array(explode('x', $options['image-size-override']))) {
							...
							if( !in_array( $options['image-size'], array( 'thumbnail', 'medium', 'large' ) ) ) {
						...
						$image = cmshowcase_get_featured_img( $post_id, $options['image-size'], $email, $use_default_image, $default_img, $dsize, $gravatar_on);
				...
				*/
				...
				$template_search  = array('{quote}','{read_more}','{review_title}','{author}','{stars}','{date}','{subtitle}'); 
				$template_replace = array($quote, $read_more, $review_title, $title, $stars, $date, $sub_title); 
					...
					$quote = '<div class="'.$quote_alignment.' summary">' . $quote . '</div>';
		...
		$this_slider_tt_params = array(
					...
					'arrow_next' => $next_arrow,
					'arrow_prev' => $prev_arrow,
					);
